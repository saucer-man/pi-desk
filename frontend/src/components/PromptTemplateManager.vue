...
<template>
  <div class="settings-content prompt-config-content">
    <div class="settings-content-header prompt-config-header">
      ...
      <div class="settings-actions">
        <button class="icon-button" type="button" :title="tr('common.refresh')" :disabled="loading || saving" @click="void loadTemplates()"><RefreshCw :size="14" :class="{ 'is-spinning': loading }" /></button>
        <button class="text-button" type="button" :disabled="saving" @click="resetEditor()"><FilePlus2 :size="14" />{{ tr("settings.addPrompt") }}</button>
      </div>
    </div>
    <p v-if="copied" class="setting-status">{{ tr("settings.copied") }}</p>
    <div v-if="loading" class="settings-empty"><RefreshCw :size="18" class="is-spinning" /><span>{{ tr("settings.loadingPrompts") }}</span></div>
    ...
    <div v-else class="prompt-manager-layout">
      <aside class="prompt-config-list" :aria-label="tr('settings.promptTemplates')">
        <button class="model-config-add-row" type="button" :class="{ 'is-active': selectedKey === 'new' }" :disabled="saving" @click="resetEditor()"><FilePlus2 :size="14" /><span>{{ tr("settings.addPrompt") }}</span></button>
        ...
        <section class="prompt-config-scope" :class="{ 'is-disabled': !projectAvailable }">
        ...
        <section class="prompt-config-scope runtime-resource-scope">
          ...
          <div v-for="prompt in runtimePrompts" :key="`${prompt.name}-${prompt.path}`" class="runtime-resource-item" :title="prompt.path">
      ...
      <form class="prompt-editor" @submit.prevent="void saveTemplate()">
        ...
        <div class="model-form-grid">
          <label class="model-field">
            ...
            <select :value="editor.scope" :disabled="saving || isExisting" @change="selectScope">
        ...
        <footer class="model-editor-footer">
          <button v-if="isExisting" class="text-button danger" type="button" :disabled="saving" @click="void deleteTemplate()"><Trash2 :size="14" />{{ deleteArmed ? tr("settings.confirmDeletePrompt") : tr("settings.deletePrompt") }}</button>
          <span />
          <button class="text-button primary" type="submit" :disabled="saving || !dirty"><Save :size="14" />{{ saving ? tr("settings.savingPrompt") : tr("settings.savePrompt") }}</button>
        </footer>
      </form>
    </div>
  </div>
