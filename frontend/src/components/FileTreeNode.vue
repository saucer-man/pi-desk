...
const props = defineProps<{ node: RepositoryTreeNode; depth?: number }>();
const emit = defineEmits<{ mention: [path: string, directory: boolean] }>();
...
<template>
  <div class="file-tree-node">
    <div class="file-tree-row" :style="{ '--tree-depth': depth || 0 }">
      ...
      <File v-else :size="14" />
      <span class="file-tree-name" :title="node.path">{{ node.name }}</span>
    ...
    <div v-if="node.directory && open" class="file-tree-children">
      <FileTreeNode v-for="child in node.children" :key="`${child.directory}-${child.path}`" :node="child" :depth="(depth || 0) + 1" @mention="forwardMention" />
    </div>
  </div>
</template>
